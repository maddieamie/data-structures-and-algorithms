...
const maxInArray = (arr) => {
  // Solution code here...alksdjflksjdf;lj
  const getMax = (a, b) => Math.max(a, b);

  let max = arr.reduce(getMax, 0);
  console.log(max);
  return max;
};
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  let found = arr.find((thing) => (character === thing.name));

  if (found !== null && found !== undefined && Object.values(found.children).length > 0) {return true}
  else {return false}
};
