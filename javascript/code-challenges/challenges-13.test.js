...
const replaceZeros = (string) => {
  // Solution code here...
  let regex = /0/g;
  let sstring = string.replaceAll(regex , 'zero');
  return sstring;
...
const validateEmail = (email) => {
  let regex = /^(\w+|\w+\.\w+?)@\w+(?:\.com|.net|.org)$/gm;

  let n = regex.test(email);
  return n;
};
