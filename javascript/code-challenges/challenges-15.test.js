...
const screenForNames = (arr) => {
  // Solution code here...
  let regex = /(?:Mr\.|Mrs\.|Ms\.|Dr.)\s(\w+)/g;
  
  let gah= arr.filter((word) => regex.test(word));
...
const toTitleCase = (arr) => {
  // Solution code here...
 let ah = arr.map(string => {return string.charAt(0).toUpperCase() + string.slice(1)});
  return ah;
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let sorted= arr.filter((character) => +character.mass > 77);
  let names = sorted.map((character) => character.name).join(' - ')
  return names;
};
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
    ...
    let boardnotes = [];
      ...
      let c1= board.map(row => row[0]).join('');
      let c2 = board.map(row => row[1]).join('');
      let c3= board.map(row => row[2]).join('');
      let go1 = board[0][0] + board[1][1] + board[2][2];
      let go2 = board[0][2] + board[1][1] + board[2][0]; 
  
  
     console.log('row1', row1, 'row2', row2, 'row3', row3, 'c1', c1, 'c2', c2, 'c3', c3, 'go1', go1, 'go2', go2);
     
     boardnotes.push(row1, row2, row3, c1, c2, c3, go1, go2);

    if (boardnotes.includes('XXX')) {
    return true;
    }
    else if (boardnotes.includes('OOO')) {
    return true;
    }
    else {return false}
    };
